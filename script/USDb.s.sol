...
contract DeployUSDBTestnet is Script {
    function run() public {
        ...
        USDb _usdb = new USDb(admin, endpoint, usdc, spct, oracle, cdPeriod, signerAddress);
...
contract Interaction is Script {
    ...
    function redeem(address _usdb, uint256 _amount, bytes calldata _signature) public {
        uint256 deployerPrivateKey = vm.envUint("PRIVATE_KEY");
        
        vm.startBroadcast(deployerPrivateKey);
        USDb(payable(_usdb)).cdRedeem(_amount, _signature);
        vm.stopBroadcast();
    }
}
